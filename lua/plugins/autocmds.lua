...
autocmd('FileType', {
  ...
  callback = function()
    ...
    vim.keymap.set('n', '<leader>r', ':w | split | resize 17 | terminal g++ % -o %< && ./%< && echo "" && read -p "Presiona Enter para cerrar..." && exit<CR>',
...
autocmd('FileType', {
  ...
  pattern = 'python',
  callback = function()
    vim.keymap.set('n', '<leader>r', ':w | split | resize 17 | terminal python3 %<CR>',
