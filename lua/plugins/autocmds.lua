...
autocmd('FileType', {
  ...
  callback = function()
    ...
    vim.keymap.set('n', '<leader>r', ':w | belowright 12split | terminal g++ % -o %< && ./%< && echo "" && read -p "Presiona Enter para cerrar..." && exit<CR> | startinsert',
...
autocmd('FileType', {
  ...
  pattern = 'python',
  callback = function()
    vim.keymap.set('n', '<leader>r', ':w | belowright 12split | terminal python3 %<CR> | startinsert',
